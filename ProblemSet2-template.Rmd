...
We use the 2017 General Social Survey(GSS) data^[* The target population includes all non-institutionalized persons 15 years of age and older, living in the 10 provinces of Canada.  
\    \    \ * The frame population is everyone who is registered combining both landline and cellular with Statistics  
\    \    \ Canada’s address registers.   
\    \    \ * The sampled population is whoever is reached via telephone.] to study the response of people in Canada's life satisfactions to multiple factors. The target population was divided into 27 strata by geographic areas, and simple random sampling without replacement of records was performed in each stratum (which means, from each stratum/group, everyone has an equal probability of being chosen). The objective of the analysis is to find and analyze some potential factors that affect life satisfaction, and to observe the most significant factor among them. __Linear model and r-squared method is used to determine the significant factors, and plots are drawn to show the linear trend or to exaplain some weakness of the analysis.__ Through the analysis, We find a strong linear relationship between self rated mental health and life satisfaction rate. Which means, people who rate themselves as "mentally healthy" are relatively more satisfied with their lives. The analysis also shows a linear relationship between physical health and life satisfaction. On the other hand, there is no overall effects of hours worked on life satisfaction level. 
...
The goal of this process is to find some factors that affect one's satisfaction on life. 

Graphical visualization of positive correlation of the mental health and life satisfaction score:

```{r echo = F, warning = F, message = F, fig.width=12, fig.height=6.5}
...
From Table 1 and Summary 1 in the result section, we find that physical health also has a significant positive linear relationship (using estimates and p-value, and r-squared). so we find physical and mental health are the two major factors that affect life satisfaction. However there is no strong relationship shown between income, hours worked and life satisfaction(from summary 1 using p-values and estimates). Hence, the result suggests that both physical and mental health are significant predictors. Between the two variables, mental health is a more important factor than physical health in terms of explaining the variability of the life satisfaction score.
...
Every statistic data study and analysis includes some biases and weaknesses. One of our weaknesses is that the predictor variables are categorical variable, which made it challenging to visualize the linear regressions model we found and show the significance between the life satisfaction score and other factors. 
...
X <- resid(mod)
qqnorm(X, main = "Figure 2. Normal Q-Q Plot")
qqline(X, col = 'red')
```
...
Normality is one of the assumptions in the linear regression model. However, the normal QQ plot above suggests that our model does not satisfy the normality assumption on the error terms. Therefore, we need take into account that the result drawn from the regression model could be misleading or biased.
