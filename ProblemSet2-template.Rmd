...
As we can observe from the output, estimated satisfaction_score increases as each of self_rated_health, self_rated_mental_health and family_income increases; however, the slope estimates for work_hours are found to be quite inconsistent, because there is a decrease in slope estimates from 0.29608 to 0.23252 in the first two intervals of work_hours, but it increases again in the thrid and fourth intervals.  
...
There are two ways to do this^[One way is to compare the standardized regression coefficients and the other way is to compare the increases in R-squared when each predictor is added to the regression model. For this analysis, specifically, we could not use the first method, because our model contains categorical predictor variable which cannot be standardized.].

In this analysis, we are going to use the method where we compute and compare the changes in$R^2$ for the last variable added to the model. This is a valid method for identifying which predictor explains the most variability in the response variable, because when a newly added predictor variable is the only difference between the two models, the associated change in $R^2$ will represent the 'goodness-of-fit'.  

To be more specific, we are going to begin with fitting a linear regression model with only one predictor variable, and then add one predictor to the model at a time to see how much $R^2$ changes when each variable is added.
...
```{r echo = F, warning = F, message = F}
tibble(Predictor = c("self_rated_health", "self_rated_health + self_rated_mental_health", "self_rated_health + self_rated_mental_health + family_income"), R_squared = c(first_R2, second_R2, third_R2), Change = c(first_R2, second_R2 - first_R2, third_R2 - second_R2))
```
...
*Woolim
our predictor variable is categorical so we couldnt
