...
We use the 2017 General Social Survey(GSS) data obatained from the CHASS website to study and analyze some potential factors affecting Canadians' life satisfactions, as well as to observe the most significant factor among them. Linear model and adjusted $R^2$^[*adjusted $R^2$ is explained in "Model" section] are used to determine the significant factors, and plots are drawn to show the linear trend or to exaplain some weakness of the analysis. Through the analysis, We find a positive linear relationships between two independent variables self_rated_mental_health, self_rated_health and a dependent variable life_satisfaction rate, which means people who rate themselves as "mentally and/or physically healthy" are relatively more satisfied with their lives; on the other hand, there is no overall effects of hours worked on life satisfaction level. 
...
#col.names = linebreak(c("ID", "satisfaction_score","self_rated_health", "self_rated_\n mental_health", "family_income", "work_hours"))) 
...
The goal of this process is to find some factors that affect one's satisfaction on life. Data cleaning for the data given from 2017 General Social Survey(GSS) data set is done in the "Data" part, but there may have some bias since many responses are not available. Therefore, We focused more on the variables with less NA responses to do the anlaysis as accurate as possible. 
...
Feder, A. (2018, April 10). Wrap column name in pdf table, from knitr::kable. https://community.rstudio.com/t/wrap-column-name-in-pdf-table-from-knitr-kable/3278/4. (apa7)
